...
```{r}
rf_5 = randomForest(theftrate ~. -fips -state - county, mtry = 5, data = theft_train)
rf_6 = randomForest(theftrate ~. -fips -state - county, mtry = 6, data = theft_train)
rf_12 = randomForest(theftrate ~. -fips -state - county, mtry = 12, data = theft_train)
rf_14 = randomForest(theftrate ~. -fips -state - county, mtry = 14, data = theft_train)
```
...
oob_errors2 %>%
  ...
  geom_line() + labs(y = "Out of Bag Error", x = "Number of Trees Used", color = "m") + theme_bw()
```
...
```{r cache=TRUE}
set.seed(471) # set seed for reproducibility
rf_12 = randomForest(theftrate ~ .-fips -state -county, mtry = 12, data = theft_train, importance = TRUE)
```
